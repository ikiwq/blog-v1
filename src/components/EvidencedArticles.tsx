...
const EvidencedArticles = (props: Props) => {
  ...
  return (
    <div className='flex flex-col md:grid grid-cols-1 xl:grid-cols-5 md:space-y-2 xl:space-y-0 xl:space-x-2'>
        <div className='col-span-3 flex flex-col py-1 main-container overflow-hidden'>
                <Link to={`/post/${props.posts[0].slug}`} className="">
                  <img src={props.posts[0].featureImage?.url} className="h-72 w-full block object-cover hover:scale-105 transition duration-500"/>
                </Link>
                <Link to={`/post/${props.posts[0].slug}`}>
                <div className='bg-neutral-900 text-white px-4 flex justify-between items-center py-3'>
                ...
                <div className='bg-neutral-900 text-sm px-4 text-neutral-300 pb-2'>
